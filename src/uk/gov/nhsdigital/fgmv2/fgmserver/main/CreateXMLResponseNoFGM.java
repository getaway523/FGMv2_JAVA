...
public class CreateXMLResponseNoFGM {
	...
	public static MessageHeader createMessageHeader(){
		...
		ResourceMetadataMap meta = new ResourceMetadataMap();
		//lastUpdated meta element not needed in FGM Query response message
		...
		event.setCode("urn:nhs:names:services:clinicals-sync:FGMQueryResponse_2_0");
		msgH.setEvent(event);
		...
		//only if No FGM Found or invalid query
		rsp.setDetails(new ResourceReferenceDt().setReference("OperationOutcome/33daadee-26e1-4d6a-9e6a-7f4af9b58933"));
...
//		Only Needed if FGM Flg = active
//		ResourceReferenceDt rr2 = new ResourceReferenceDt();
//		rr2.setReference("Flag/8cb73a48-090d-469a-a2b2-04f1e6b11ea2");
...
//		msgH.setData(rr2list);
	...
	public static OperationOutcome createOperationOutcome(){
		...
		OperationOutcome opout = new OperationOutcome();
		opout.setId("33daadee-26e1-4d6a-9e6a-7f4af9b58933");
		ResourceMetadataMap meta = new ResourceMetadataMap();
		meta.put(ResourceMetadataKeyEnum.PROFILES, "http://fhir.nhs.net/StructureDefinition/http://fhir.nhs.net/StructureDefinition/spine-operationoutcome-1-0");
		opout.setResourceMetadata(meta);
		
		//issue severity broken. Needs latest version of HAPI
		
		//Issue iss = new Issue();
		//iss.setIdentifier("14daadee-26e1-4d6a-9e6a-7f4af9b58877");
//		iss.setSeverity(IssueSeverityEnum.WARNING);
//		iss.setCode(IssueTypeEnum.BUSINESS_RULE);
//		opout.addIssue(iss);
//		
		opout.addIssue().setDiagnostics("No FGM Record Found").addLocation("test");
